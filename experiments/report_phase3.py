...
============================
Generate an HTML report for Phase 3 (Feature Set Sweep) results.
...
GROUPS = {
    "09_feat_compact": "Compact<br><small>Returns, Range, vol, log_ret</small>",
    "10_feat_full":    "Full<br><small>All 13 features</small>",
}
...
def top_annual_table(df: pd.DataFrame, n: int = 15) -> str:
    ...
    return f"""
    <h2>10. Top {n} Configurations by HMM OOS Annual Return (CAGR%)</h2>
    <div class="scroll">
    <table>
      <thead>
        <tr>
...
def load_windows() -> pd.DataFrame:
    """Load all individual walk-forward window rows for phase 3."""
    pattern = os.path.join(ROOT, 'results', 'phase_3', '*', '*', '*', '*_results.csv')
    frames = []
...
def findings_section(df: pd.DataFrame) -> str:
    ...
    return f"""
    <h2>13. Key Findings &amp; Recommendation</h2>
...
def build_report(master_path: str, out_path: str):
    ...
    wins         = load_windows()
    best_win_sharpe = best_windows_table(wins, 'hmm_oos_sharpe', 11,
                         'Best 15 Windows by HMM OOS Sharpe',
                         'PCA', 'group_id', GROUPS)
    best_win_annual = best_windows_table(wins, 'hmm_oos_annual', 12,
