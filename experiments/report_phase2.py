#!/usr/bin/env python3
"""
experiments/report_phase2.py
============================
Generate an HTML report for Phase 2 (K / Hidden-State Count Sweep) results.

Usage:
    python experiments/report_phase2.py [--out results/phase_2_report.html]
"""
...
def k_summary_table(df: pd.DataFrame) -> str:
    """Per-K aggregate summary (mean across all strategies & timeframes)."""
    best_sharpe = -1e9
    best_gid = None
    summary_rows = []
    ...
    return f"""
    <h2>1. K Summary (mean across all strategies &amp; timeframes)</h2>
    <div class="scroll">
    <table>
      <thead>
        <tr>
          <th>K</th><th>Rows</th><th>Win%</th>
...
def strategy_heatmap(df: pd.DataFrame, metric: str, metric_label: str,
    ...
    """Cross-table: rows = strategy, columns = K × timeframe."""
    ...
    return f"""
    <h2>{section_num}. {metric_label} by Strategy &amp; K</h2>
    <p class="note">{note}</p>
    <div class="scroll">
    <table>
      <thead>{header}</thead>
...
def win_rate_table(df: pd.DataFrame) -> str:
    ...
    return f"""
    <h2>7. Window Win Rate (% windows HMM improved OOS Sharpe)</h2>
...
def findings_section(df: pd.DataFrame) -> str:
    ...
    for gid, label in GROUPS.items():
        sub = df[df["group_id"] == gid]
        if not sub.empty:
            k_sharpes[label] = sub["hmm_oos_sharpe_mean"].mean()

    best_label = max(k_sharpes, key=k_sharpes.get)
    ...
    ranking_rows = "".join(
        ...
        for k, v in sorted(k_sharpes.items(), key=lambda x: -x[1])
    )
    ...
    <h3>9.2 Winner: <span style="color:#1a7a1a">{best_label}</span></h3>
    <p>More hidden states allow the HMM to distinguish finer market regimes, but
    too many states risk overfitting to noise. The winning K represents the
    best bias-variance trade-off on these walk-forward windows.</p>

    <h3>9.3 Strategies benefiting from {best_label}</h3>
    <p>Positive Δ Sharpe in at least one timeframe: {improved_str}</p>

    <h3>9.4 Strategies not benefiting</h3>
    <p>Consistently negative Δ Sharpe: {hurt_str}</p>

    <h3>9.5 Recommendation for Phase 3</h3>
    <p>Proceed with <strong>mode = score</strong> and <strong>{best_label}</strong>.
    Update <code>groups.yaml</code> Phase 3 entries accordingly and sweep
    PCA dimensionality (none, 3, 4).</p>
    """
...
.header { background: #1b5e20; color: #fff; padding: 28px 40px; }
.header h1 { margin: 0; font-size: 1.8em; }
.header p { margin: 6px 0 0; opacity: 0.85; }
.content { padding: 32px 40px; max-width: 1600px; }
h2 { border-bottom: 2px solid #1b5e20; padding-bottom: 6px; color: #1b5e20; margin-top: 40px; }
h3 { color: #333; }
table { border-collapse: collapse; font-size: 0.85em; width: 100%; margin-bottom: 16px; }
th { background: #1b5e20; color: #fff; padding: 7px 10px; text-align: left; white-space: nowrap; }
...
code { background: #e8f5e9; padding: 1px 4px; border-radius: 3px; font-size: 0.9em; }
"""
...
<head>
  ...
  <title>Phase 2 Report — K Sweep</title>
  <style>{CSS}</style>
</head>
<body>
  <div class="header">
    <h1>Phase 2 Report &mdash; K (Hidden-State Count) Sweep</h1>
    <p>SPY &amp; QQQ &nbsp;|&nbsp; 2010–2026 &nbsp;|&nbsp; 13 strategies &nbsp;|&nbsp;
       IS 2yr&amp;3yr / OOS 1yr walk-forward &nbsp;|&nbsp; Mode: Score (Phase 1 winner) &nbsp;|&nbsp;
       K ∈ {{2, 3, 4, 6}}</p>
