...
def load_windows() -> pd.DataFrame:
    """Load all individual walk-forward window rows for phase 4."""
    pattern = os.path.join(ROOT, 'results', 'phase_4', '*', '*', '*', '*_results.csv')
    frames = []
...
def best_windows_table(wins: pd.DataFrame, rank_col: str, section_num: int,
    ...
    for _, r in top.iterrows():
        ...
        rows += f"""<tr>
          <td>{GROUP_SHORT.get(r.get(group_col), r.get(group_col, ''))}</td>
          <td>{r['strategy']}</td>
          <td>{r['timeframe']}</td>
...
def build_report(master_path: str, out_path: str):
    ...
    best_win_sharpe = best_windows_table(wins, 'hmm_oos_sharpe', 11,
                         ...
                         'Feature Set', 'group_id', GROUP_SHORT)
